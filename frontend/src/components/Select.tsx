        ...
        <div ref={ref}>
            <div onClick={() => setShow(!show)} className="hover:cursor-pointer w-[141px] h-8 px-2 py-1 bg-white rounded-lg border border-gray-300 justify-start items-center gap-2 inline-flex">
                {icon && <Image
                    ...
                    alt="Icon Image"
                />}
                <div className={`${value ? "text-neutral-600" : "text-zinc-400"} text-sm font-normal font-['Rubik'] grow shrink basis-0`}>{value ? options[value - 1] : placeholder}</div>

                <NavArrowDown className="relative" width={24} height={24} />
                {/* <div className="text-zinc-400 text-sm font-normal font-['Rubik']">Please Select.</div>
                <div className="w-6 h-6 px-1.5 justify-center items-center flex" /> */}
            </div>
            <div style={{ display: show ? "block" : "none" }} className="w-[141px] font-['Rubik'] custom-select absolute bg-white border border-gray-300 rounded-sm py-2">
